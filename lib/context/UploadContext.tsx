...
type UploadContextValue = {
  uploads: UploadItem[];
  storageVersion: number; // bumped after each upload batch — Sidebar watches this
...
async function collectFiles(entry: FileSystemEntry, basePath = ""): Promise<FileWithPath[]> {
  ...
  if (entry.isDirectory) {
    ...
    const nested = await Promise.all(children.map((c) => collectFiles(c, currentPath)));
    return nested.flat();
  }
...
export function UploadProvider({ children }: { children: ReactNode }) {
  ...
  // Auto-dismiss 4 s after all uploads finish
  useEffect(() => {
  ...
  // ── Internal helpers (plain functions — no stale closure risk) ─────────────

  const addToQueue = (items: UploadItem[]) => {
  ...
  // Upload one file and update its queue row
  const uploadOne = async (file: File, uid: string, folderId: string, currentFolderId: string) => {
  ...
  // Create folder tree (shallow-first) and return path→id map
  const createFolderTree = async (
    ...
    for (const dirPath of sorted) {
      ...
      try {
        ...
        if (!parentPath) setFolders((prev: any) => [folder, ...prev]);
      } catch {
  ...
  return (
    <UploadContext.Provider value={{ uploads, storageVersion, uploadFiles, uploadFolderFileList, uploadFromDrop }}>
      {children}

      {uploads.length > 0 && (
        <div className="fixed bottom-4 right-4 z-[9999] w-72 rounded-xl border border-border bg-card shadow-2xl shadow-black/40 overflow-hidden">
          {/* Header */}

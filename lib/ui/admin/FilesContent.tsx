...
export const FilesContent = ({ initialFiles, initialTotal }: Props) => {
  ...
  useEffect(() => {
    if (page === 1) {
      fetchFiles(search, 1, showTrashed, true);
    }
      ...
      <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
                ...
                <tr key={file._id ?? i} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3">
                    <a
                      ...
                      className="flex items-center gap-2 text-blue-600 hover:underline max-w-[200px]"
                    >
                  ...
                  <td className="px-4 py-3 text-gray-500">
                    {file.userId ? (
                      <div>
                        <p className="font-medium text-gray-700">{file.userId.name}</p>
                        <p className="text-xs text-gray-400 truncate max-w-[160px]">{file.userId.email}</p>
                      </div>
                    ) : (
        ...
        <div ref={sentinelRef} className="py-3 flex justify-center">
          ...
          {!hasMore && files.length > 0 && (
            <p className="text-xs text-gray-400">All files loaded</p>
          )}
        </div>
      </div>
    </div>
